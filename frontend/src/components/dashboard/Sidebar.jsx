import { Link, NavLink, useLocation } from 'react-router-dom'
import { useAuth } from '../../context/AuthContext'

function Sidebar() {
  const { user, logout } = useAuth()
  const location = useLocation()
  const role = user?.role || 'user'
  const currentPath = location.pathname

  const userLinks = [
    { name: 'Home', icon: '🏠', path: '/portal/home' },
    { name: 'Upload Evidence', icon: '📤', path: '/portal/upload' },
    { name: 'Insights', icon: '📊', path: '/portal/insights' },
    { name: 'Credit Profile', icon: '🧠', path: '/portal/profile' },
    { name: 'Trust Status', icon: '🛡️', path: '/portal/status' },
    { name: 'History', icon: '📁', path: '/portal/history' }
  ]

  const adminLinks = [
    { name: 'System KPIs', icon: '📈', path: '/admin/overview' },
    { name: 'Fraud Radar', icon: '🚨', path: '/admin/fraud' },
    { name: 'Evidence Audit', icon: '📥', path: '/admin/review' },
    { name: 'User Governance', icon: '👥', path: '/admin/users' },
    { name: 'Model Integrity', icon: '🤖', path: '/admin/models' }
  ]

  const isPathAdmin = currentPath.startsWith('/admin')
  // Show links based ONLY on the current route prefix
  // Since DashboardLayout enforcer blocks users from wrong paths, 
  // we just show the relevant links for the path we are on.
  const links = isPathAdmin ? adminLinks : userLinks

  return (
    <aside className="hidden lg:flex w-72 flex-col bg-surface-container-low border-r border-white/5 relative z-20 h-screen">
      <div className="flex items-center gap-3 p-8 border-b border-white/5">
        <Link to="/" className="flex items-center gap-3 group">
           <div className="grid h-12 w-12 place-items-center rounded-2xl bg-surface-container border border-white/5 shadow-premium ring-1 ring-white/10 group-hover:scale-110 transition-transform overflow-hidden">
              <img src="/logo.png" alt="Q" className="h-full w-full object-cover scale-110" />
           </div>
           <div>
              <p className="font-display text-2xl font-black text-white italic tracking-tighter leading-none">Quirass</p>
              <p className="text-[9px] font-black uppercase text-on-surface-variant tracking-widest italic opacity-40 group-hover:text-primary transition-colors">← Exit Portal</p>
           </div>
        </Link>
      </div>

      <nav className="flex-1 p-6 space-y-2 overflow-y-auto">
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-primary mb-6 px-4 italic opacity-50 underline decoration-primary/20">
          {isPathAdmin ? 'Control Intelligence Hub' : 'Citizen Evidence Portal'}
        </p>
        {links.map((link) => (
          <NavLink
            key={link.path}
            to={link.path}
            className={({ isActive }) =>
              `flex items-center gap-4 px-4 py-4 rounded-2xl transition-all duration-300 group ${
                isActive 
                  ? 'bg-primary text-white shadow-premium ring-1 ring-white/10' 
                  : 'text-on-surface-variant hover:bg-white/5 hover:text-white'
              }`
            }
          >
            <span className="text-xl group-hover:scale-110 transition-transform">{link.icon}</span>
            <span className="text-xs font-black uppercase tracking-widest italic">{link.name}</span>
          </NavLink>
        ))}
      </nav>

      <div className="p-6 border-t border-white/5 space-y-4">
        <Link 
           to="/"
           className="flex items-center justify-center gap-2 w-full p-4 rounded-2xl bg-white/5 border border-white/10 text-on-surface-variant text-[10px] font-black uppercase tracking-widest hover:text-white hover:bg-white/10 transition-all italic"
        >
           <span>← Main Infrastructure</span>
        </Link>
        <button
          onClick={logout}
          className="flex items-center justify-center gap-2 w-full p-4 rounded-2xl bg-error/10 text-error text-[10px] font-black uppercase tracking-widest hover:bg-error/20 transition-all italic shadow-premium"
        >
          Logout Session
        </button>
      </div>
    </aside>
  )
}

export default Sidebar
